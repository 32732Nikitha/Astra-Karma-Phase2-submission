/* ──────────────────────────────────────────────────────────
   BHIMA ASTRA — ML Pipeline Engine (JavaScript port)
...
function runRuleEngine(features) {
  const flags = [];

  // Rule 1: GPS / Cell Tower Mismatch
  if (parseFloat(features.gps_tower_delta) > THRESHOLDS.GPS_DELTA) {
    flags.push('gps_mismatch');
  }
  // Rule 2: No Motion (stationary spoofer)
  if (parseFloat(features.accelerometer_variance) < THRESHOLDS.ACCEL_VARIANCE) {
    flags.push('no_motion');
  }
  // Rule 3: Timing Anomaly (bot-speed filing)
  if (parseFloat(features.claim_response_time_sec) < THRESHOLDS.RESPONSE_TIME) {
    flags.push('timing_anomaly');
  }
  // Rule 4: Device Blacklist
  if (parseInt(features.device_flagged) === 1) {
    flags.push('device_blacklist');
  }

  const rule_score   = parseFloat((flags.length / 4).toFixed(4));
  const rule_decision = flags.length >= THRESHOLDS.MIN_FLAGS ? 'REVIEW' : 'PASS';
...
function _normalize(value, minVal, maxVal, invert = false) {
...
function computeBehaviorScore(features) {
  ...
  const locationJump = parseFloat(features.location_jump_flag || 0);
...
function computeFinalScore(tabularProb, clusterScore, clusterSize) {
...
function computePercentile(finalScore) {
...
function makeFraudDecision(tabularProb, clusterScore, clusterSize, behaviorScore, ruleScore, features, workerName) {
  ...
  let decision, fraudFlag, payoutStatus, holdDuration;
  if (percentile >= 0.90) {
    decision = 'BLOCK'; fraudFlag = 1;
    payoutStatus = 'ON_HOLD'; holdDuration = null;
  } else if (percentile >= 0.70) {
    decision = 'REVIEW'; fraudFlag = 1;
    payoutStatus = 'PARTIAL_RELEASE'; holdDuration = '48h';
  } else {
    decision = 'APPROVE'; fraudFlag = 0;
    payoutStatus = 'FULL_RELEASE'; holdDuration = null;
  }
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
function generateFeatureImportance(features, behaviorResult) {
  const raw = [
    ...
    { name: 'location_jump_flag',       val: parseFloat(features.location_jump_flag || 0) },
    { name: 'claim_response_time_sec',  val: Math.min(1, Math.max(0, 1 - features.claim_response_time_sec / 600)) },
    { name: 'device_flagged',           val: parseFloat(features.device_flagged || 0) }
  ];
...
function generateMotionTrace(accelVariance, n = 22) {
  const points = [];
  for (let i = 0; i < n; i++) {
