...
def run_fraud_pipeline(
    ...
    if tabular_model is not None and fraud_features is not None:
        row = pd.DataFrame([features]).reindex(
            columns=fraud_features, fill_value=0)
        row = row.select_dtypes(include=[np.number]).fillna(0)
        tabular_prob = float(
            tabular_model.predict_proba(row)[0][1])
    else:
        # Fallback: use rule + behavior as proxy
        tabular_prob = float(
            np.clip(rule_score * 0.6 + behavior_score * 0.4, 0, 1))
        logger.warning(
    ...
    if skip_heavy:
        ...
        decision = make_fraud_decision(
            tabular_prob   = 0.0,
            cluster_score  = cluster_score,  # ✅ Keep actual graph score
            cluster_size   = cluster_size,
            behavior_score = behavior_score,
            rule_score     = rule_score,
            worker_id      = worker_id,
    ...
    # Always build the EXACT same dictionary structure
    result = {
        # Stage outputs
